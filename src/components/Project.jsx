...
const Project = () => {
    ...
    useEffect(()=>{
        axios.get("https://api.github.com/users/kshavp")
        .then((res)=>{
        ...
        <div className="md:flex flex-row justify-around">
            <div className="m-8 md:m-2 flex flex-col md:w-1/2">
                {PROJECTS.map((project,index)=>(
                    <div key={index} className="mb-12 md:px-12 md:w-lg">
                        {index === 0 ? (<p className="text-2xl font-bold">{project.title} <p className="font-thin text-white text-lg mb-4">2nd Position, Inter-Department Hackathon</p></p>) :
                        (<p className="text-2xl font-bold">{project.title}</p>)}
                        <p className="opacity-80">{project.description}</p>
                        <div className="flex flex-wrap gap-4 ">
                            {project.technologies.map((tech,index)=>(
                                <span key={index} className="m-0 md:m-2 md:ml-0 bg-neutral-800 hover:bg-neutral-700 p-2
